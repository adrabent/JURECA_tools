...
atdb_service -o executor --interval 30  --config ${ATDB_CONFIG} --atdb_host ${ATDB_HOST} --service_filter ${SERVICE_FILTER} -v2 >> ${LOG_DIR}/atdb_executor.log 2>> ${LOG_DIR}/atdb_executor.err &
atdb_service -o aggregator --interval 30  --config ${ATDB_CONFIG} --atdb_host ${ATDB_HOST} --service_filter ${SERVICE_FILTER} -v2 >> ${LOG_DIR}/atdb_aggregator.log 2>> ${LOG_DIR}/atdb_aggregator.err &
